...
@ManagedBean
@ViewScoped
public class AnalyseScController extends AbstractAnalyseSc implements Serializable {
    ...
    private BarChartModel initBarModel() {
        try {
            ...
            for (AnalyseSCB a : analyses) {
                series1.set(a.getAction().getCode(), a.getMontantDisponible());
                series2.set(a.getAction().getCode(), a.getMontantProgramme());
            }
    ...
    private PieChartModel initPieCharModel() {
        try {
            ...
            for (AnalyseSCB a : analyses) {
                model.set(a.getAction().getCode(), a.getMontantDisponible());
            }
        ...
        if (idPeriode != null && idPeriode > 0) {
            ...
            if (!list.isEmpty()) {
                ...
                for (ContratMoyens c : list) {
                    if (!actions.contains(c.getMoyens().getIdtache().getIdactivite().getIdaction())) {
                        actions.add(c.getMoyens().getIdtache().getIdactivite().getIdaction());
                    }
                }
                int i = 0;
    ...
    public Map getData(Action a, List<ContratMoyens> list) {
        Map map = new HashMap();
        List<ContratMoyens> cms = new ArrayList<>();
        double montant = 0;
        for (ContratMoyens c : list) {
            if (c.getMoyens().getIdtache().getIdactivite().getIdaction().equals(a)) {
                montant += c.getCt();
                cms.add(c);
            }
        }
